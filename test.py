...
def reset_arduino():
    ...
    try:
        ...
        initialize_serial()
        logger.info("Arduino connection reset successful")
        time.sleep(2)  # Wait for Arduino to reboot
    except serial.SerialException as e:
        logger.error(f"Error resetting Arduino connection: {e}")
...
def get_data():
    ...
    if raw_data is not None:
        ...
        # Try reading data again after reset
        raw_data = read_serial_data()
        if raw_data is not None:
            eval_result = raw_data >= config['serial']['threshold']
            return jsonify({"eval": eval_result, "raw": raw_data, "reset": True})
        else:
            return jsonify({"error": error_message, "eval": False, "raw": None, "reset": True}), 500
...
if __name__ == '__main__':
    initialize_serial()  # Initialize serial connection when the app starts
